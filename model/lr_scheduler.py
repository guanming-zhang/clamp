...
class LinearWarmupCosineAnnealingRestartLR(_LRScheduler):
    """Sets the learning rate of each parameter group to follow a linear warmup schedule with cosine restart
        between warmup_start_lr and base_lr followed by a cosine annealing schedule
        between base_lr and eta_min.
    ...
    def __init__(
        ...
        Args:
            ...
            warmup_epochs (int): Maximum number of iterations for linear warmup
            max_epochs (int): Maximum number of iterations
            n_restart (int): Number of times for the restart, each restart last for (max_epochs-warmup_epochs)//n_restart epochs
            warmup_start_lr (float): Learning rate to start the linear warmup. Default: 0.
            eta_min (float): Minimum learning rate. Default: 0.
            last_epoch (int): The index of last epoch. Default: -1.
        """
        print((max_epochs - warmup_epochs) % n_restart)
        assert warmup_epochs >= 2, "[LinearWarmupCosineAnnealingRestartLR]--warmup_epochs must be ≥ 2"
        assert max_epochs > warmup_epochs, "[LinearWarmupCosineAnnealingRestartLR]--max_epochs must exceed warmup_epochs"
        assert n_restart >= 1, "[inearWarmupCosineAnnealingRestartLR] -- n_restart must be ≥ 1"
        assert (max_epochs - warmup_epochs) % n_restart ==0, "[inearWarmupCosineAnnealingRestartLR] --(max_epochs - warmup_epochs) % n_restart !=0"
        self.warmup_epochs = warmup_epochs
        self.max_epochs = max_epochs
        self.warmup_start_lr = warmup_start_lr
        self.eta_min = eta_min
        self.n_restart = n_restart
        self.T = (max_epochs - warmup_epochs) // n_restart
    ...
    def _get_closed_form_lr(self) -> List[float]:
        ...
        return [
            ...
            + 0.5
            * (base_lr - self.eta_min)
            * (1 + math.cos(math.pi* epoch/ self.T))
            for base_lr in self.base_lrs
        ]